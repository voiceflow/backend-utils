...
import { RateLimitClient } from '../../src/clients/rateLimit';

describe('rateLimit client unit tests', () => {
  beforeEach(() => {
    sinon.restore();
  });
  ...
  it('constructor', async () => {
    ...
    const limiter = RateLimitClient('foo-service', redis as any, config as any);
    ...
    expect(_.get(limiter.public, '_keyPrefix')).to.eql('foo-service-rate-limiter-public');
    ...
    expect(_.get(limiter.private, '_keyPrefix')).to.eql('foo-service-rate-limiter-private');
  });
});
